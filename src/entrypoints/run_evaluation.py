...
def main():
    ...
    for model in settings.keys():
        if settings[model]['enable']:
            ...
            extractor_model = settings[model].get('extractor_model', DEFAULT_EXTRACTOR_MODEL)             # Get extractor_model from settings, fallback to DEFAULT_EXTRACTOR_MODEL if not found
